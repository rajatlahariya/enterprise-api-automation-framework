...
public class JsonNodeUtilsTest {
    ...
    public void shouldExtractSupportedJsonValues() throws Exception {
        ...
        assertThat(
                JsonNodeUtils.getText(node, "name"),
                equalTo("Rajat")
        );

        assertThat(
                JsonNodeUtils.getInteger(node, "age"),
                equalTo(29)
        );

        assertThat(
                JsonNodeUtils.getBoolean(node, "active"),
                equalTo(true)
        );

        assertThat(
                JsonNodeUtils.getLocalDateTime(node, "createdAt"),
                equalTo(LocalDateTime.parse(
                        "2026-07-06T13:36:27.564827"
                ))
        );
    }
    ...
    @Test
    public void shouldReturnNullWhenOptionalFieldIsMissing()
            throws Exception {

        JsonNode node = OBJECT_MAPPER.readTree("{}");

        assertThat(
                JsonNodeUtils.getText(node, "missing"),
                nullValue()
        );
    }
